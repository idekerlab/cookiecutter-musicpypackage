...
This is an advanced developer section that describes creation of a new release for {{ cookiecutter.project_slug }} and deploying
...
Be sure to update the version in ``{{ cookiecutter.project_slug }}/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' {{ cookiecutter.project_slug }}/__init__.py
...
From {{ cookiecutter.project_slug }} source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- {{ cookiecutter.project_slug }}-{{ cookiecutter.version }}-py2.py3-none-any.whl
    `-- {{ cookiecutter.project_slug }}-{{ cookiecutter.version }}.tar.gz
...
#. From https://github.com/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }} click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/{{ cookiecutter.project_slug }} and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi {{ cookiecutter.project_slug }}
